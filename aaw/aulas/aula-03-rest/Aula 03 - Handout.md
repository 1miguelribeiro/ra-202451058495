...
> **Nomes:** Miguel Ribeiro Muniz Morais    **Turma:** Quinta-feira noite  **Data:** 20 / 08 / 2026
...
Utilizando verbo no endpoint: "get". Utilizando POST no lugar do GET. Não utilizou paginação, response muito grande.

2. Seu redesenho 

Método + rota + status codes ->  GET /api/alunos, 200 + retorna os alunos cadastrados
...
Verbo "deletar, utilizando GET no lugar do DELETE.

2. Seu redesenho 

(método + rota + status codes): DELETE /alunos/7, 204 (deleta o aluno de id 7)
...
O status 200, era pra ser 201 indicando que foi criado um novo objeto.

2. Seu redesenho (método + rota + status codes):

POST /api/alunos -> 201 + header location: /api/alunos/{id}
...
Aninhamento excessivo, o app precisa saber de 5 IDs pra buscar 1 recurso.

2. Seu redesenho (método + rota + status codes):

GET /api/disciplinas/12 -> 200. Daria para passar a matricula pelo body e acessar as informações.
...
O status 200 no lugar do 404.

2. Seu redesenho (método + rota + status codes):

GET /api/alunos/7/matriculas -> 200 ou 404 se não existe.
...
Verbo na rota "atualizar". Dados da query na string, deveriam vir pelo body.

2. Seu redesenho (método + rota + status codes):

PATCH /api/alunos/7/disciplinas/12/nota + body contendo a nota.
...
Versionar a API (v1,v2...), deixando o app mobile utilizando a v1 e criando a v2 atualizada.